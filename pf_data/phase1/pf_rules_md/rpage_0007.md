...
| 战栗或惊惧（Shaken or        frightened） | –2 | –2 |
| 挤在狭窄空间（Squeezing        through a space） | –4 | –4 |
| ※1被纠缠生物敏捷属性承受-4减值，这可能影响他的攻击检定。       ※2目标的AC失去敏捷加值。       ※3大多数远程武器无法在使用者俯卧时攻击；不过俯卧使用弩、火器和手里剑则无减值。 |
...
| 拥有掩护（Behind        cover） | +4 | +4 |
| 目盲（Blinded） | –2（※1） | –2（※1） |
| 拥有隐蔽或隐形（Concealed or invisible） | 见隐蔽 |
| 畏缩（Cowering） | –2（※1） | –2（※1） |
| 被纠缠（Entangled） | +0（※2） | +0（※2） |
| 措手不及（Flat-footed） | +0（※1） | +0（※1） |
...
在确认你的目标对于你的远程攻击是否具有遮掩时，需要从自身占据方格中选择一个角，从那个角往目标占据方格的每个角画出连线，若其中有任何连线穿过的方格或边线有可以为目标提供掩护的遮挡物，或穿过的格子有任何生物占据，那么你的攻击目标视为具有掩蔽（+4AC）。


你对相邻的目标做近战攻击时，如果自身占据方格的每个角与目标占据方格的每个角，其中有任何连线被墙壁（即使是矮墙）阻隔，则目标具有掩蔽，当你对一个并非相邻的目标做近战攻击时（比如使用长触及武器攻击），那使用远程武器的判断方式决定目标有没有掩蔽。。


###### 低矮的障碍物与掩蔽（Low Obstacles and 
Cover）
...
###### 隐蔽与隐匿检定（Concealment and Stealth 
Checks）
