"""
fix_feat_page52.py — 专长52 跨行断裂标题源数据归一（P0 级，est=13 got=4）
...
用法：python3 fix_feat_page52.py
"""

import re
import sys

PATH = "pf_rules_md_organized/专长52.md"
...
def main() -> int:
    ...
    lines = raw.split(b"\n")
    n = len(lines)
    assert n == 114, f"预期 114 行，实际 {n} 行——行号假设失效，中止"
    assert lines[0].startswith(b"**\xe6\x83\x8a\xe5\xa5\x87\xe5\xa4\xa7\xe5\xb8\x88"), \
        f"L1 非惊奇大师：{lines[0][:40]!r}"
