"""processors/profession.py — 职业文件处理器

从 vectorization_prep_profession.py 机械迁移（CP1，纯复制不改行为）：
9 来源 Provider 链 / detect_book / 变体主表 / 女巫补充表 / 7 特化 Processor /
内容哈希去重 / process_profession_corpus 编排。

ProfessionProcessor 覆写 FileProcessor.process() 走老脚本完整管道
（clean → split → expand → merge → build_chunk → keep），输出按 §3 字段
映射落 StandardChunk（14 字段）；chunk_id/doc_id 沿用老产物口径（sha256 /
相对路径 doc_id），保证与新老产物等价性。
"""
import json
import hashlib
import re
...
from vectorizer.formats.profession import (
    INPUT_DIR,
    MAX_CHUNK_CHARS,
    MD_MAPPING_JSON,
    MIN_CHUNK_CHARS,
...
from vectorizer.processors.base import Chunk as StandardChunk
from vectorizer.processors.base import FileProcessor as FrameworkFileProcessor
# CP3：职业 finalize 只补 class_archetype_index.json 等价索引写出（不修改 chunk 数据）
from vectorizer.postprocess.profession_index import ProfessionIndexWriter

# ---------------------------------------------------------------------------
# 常量
# ---------------------------------------------------------------------------
# 旧脚本产物目录（保留仅参考；pipeline 实际输出由 --output 决定）
OUTPUT_DIR = Path(__file__).resolve().parent.parent.parent / "vectorization_prep_profession"
...
def resolve_book_candidate(ctx: SourceContext) -> Optional[SourceCandidate]:
    """按置信度聚合所有 Provider 的候选来源，返回最佳候选（含置信度）。"""
...
def resolve_book_sources(ctx: SourceContext) -> Tuple[str, str, str]:
    ...
    best = resolve_book_candidate(ctx)
    if best is None:
        return "未知来源", "?", "?"
    return best.book
...
def detect_book(
    ...
    confidence_out: Optional[dict] = None,
) -> Tuple[str, str, str]:
    """基于 Provider 框架推断来源书（兼容旧接口）。

    confidence_out: 若传入 dict，则在返回前写入 {"confidence": <int>}，
    供调用方捕获来源置信度（对应 StandardChunk.source_confidence）。
    """
    ...
    if confidence_out is not None:
        confidence_out["confidence"] = best.confidence if best is not None else 0
    if best is None:
        return "未知来源", "?", "?"
    return best.book
...
def _looks_like_class_table(text: str) -> bool:
    ...
    KN222：进阶职业（PoP 等）用简写表头 `| 等级 | BAB | 强韧 | 反射 | 意志 |`，
    全称表头（CRB：基本攻击加值/强韧豁免/反射豁免/意志豁免）之外需同时认简写
    形态，否则 keep() 的纯表格过滤器（is_pure_table_or_nav）会把含先决条件的
    主块整块丢弃。简写锚定 等级+BAB+强韧+反射+意志 五列（容忍空列 `||` 形态）；
    召唤师魅影表（HD/BAB/豁免/技能）等非职业表缺强韧/反射/意志列，天然不匹配。
    """
    t = re.sub(r"[ 　]", "", text)
    if (
        "基本攻击加值" in t
        ...
        return True
    # 简写表头：| 等级 | BAB | 强韧 | 反射 | 意志 |（去空格后连续匹配，容忍 `||` 空列）
    return bool(re.search(r"\|等级\|+BAB\|+强韧\|+反射\|+意志", t))
...
class FileProcessor:
    ...
    def __init__(
        ...
        # 最近一次来源解析的最佳置信度（StandardChunk.source_confidence 数据源）
        self._last_source_confidence = 0
    ...
    def build_chunk(
        ...
        return Chunk(
            ...
            char_count=len(text),
            text=text,
            source_confidence=getattr(self, "_last_source_confidence", 0),
        )
    ...
    def resolve_source(
        ...
        # confidence_out 捕获 Provider 链聚合出的最佳置信度，落 StandardChunk.source_confidence。
        confidence_out: dict = {}
        result = detect_book(
            self.rel_path,
            source_markers,
        ...
        self._last_source_confidence = confidence_out.get("confidence", 0)
        return result
...
# ---------------------------------------------------------------------------
# 框架适配层：ProfessionProcessor（职业类目，vectorizer/ 统一入口）
# ---------------------------------------------------------------------------
# 老脚本（vectorization_prep_profession.py）的块层逻辑原样保留在上文——
# 7 特化 Processor（Oracle/Warpriest/Summoner/Shifter/Sorcerer/Witch/Paladin）、
# 9 来源 Provider 链、clean/split/expand/merge/keep 全链。本类只做框架适配：
#   - load_resources 复现老 process_profession_corpus 的启动装配（canonical
#     去重 + 变体主表/巫术来源表预扫描）
#   - process 逐文件走 create_processor + run（非 canonical 副本跳过）
#   - _to_standard 按《职业法术模块重整方案》§3 映射落 StandardChunk
# POSTPROCESSORS 仅含 ProfessionIndexWriter（CP3）：职业 chm_toc_path/source
# 已由老逻辑全覆盖，finalize 无需改 chunk 数据；索引写出是纯补充产物（等价
# 索引，见 postprocess/profession_index.py），不影响 chunk 逐字段等价性。


def _profession_file_condition(file_path: Path) -> bool:
    """职业文件判断：位于 pf_rules_md_organized/职业/ 目录下的 .md 文件。

    pipeline --input 传根目录时，靠本条件把遍历范围收窄到职业子目录，
    避免对全树非类目文件误扫（与 diagnostics only= 参数同口径）。
    """
    try:
        file_path.resolve().relative_to(INPUT_DIR)
    except ValueError:
        return False
    return True


class ProfessionProcessor(FrameworkFileProcessor):
    """职业文件处理器 —— 老脚本逻辑的框架适配层。"""

    # CP3：仅补 class_archetype_index.json 等价索引写出（职业来源/toc 由老逻辑
    # 全覆盖，chunk 数据无需 finalize 改写；索引为审计用补充产物）
    POSTPROCESSORS = [ProfessionIndexWriter]
    ...
    def __init__(self, source_resolver: "SourceResolver"):
        # format_handler 仅占位：process() 覆写走老块层逻辑，不走框架模板链
        super().__init__(source_resolver=source_resolver, format_handler=ProfessionFormat())
        self._ctx: Optional[Dict[str, Any]] = None
        self._canonical_paths: Optional[Set[Path]] = None

    # ---- ABC 抽象方法：process() 覆写后模板链永不触发，三者显式拒用 ----
    def build_chunk(self, template: StandardChunk, item: dict) -> StandardChunk:
        raise NotImplementedError("职业类目走老块层逻辑（process 覆写），不经过模板 build_chunk")

    def resolve_source(self, chunk: StandardChunk, ctx: object, item: dict) -> StandardChunk:
        raise NotImplementedError("职业类目来源由老 9 Provider 链解析（resolve_book_sources），不经过模板 resolve_source")

    def infer_metadata(self, chunk: StandardChunk, item: dict) -> StandardChunk:
        raise NotImplementedError("职业类目元数据由老 build_chunk 平铺（_to_standard 落 metadata），不经过模板 infer_metadata")

    def load_resources(self, input_dir: Path) -> None:
        """复现老脚本 process_profession_corpus 的启动装配。

        canonical 去重必须固定在职业目录（INPUT_DIR）扫描——老脚本
        process_profession_corpus 的 input_dir 就是职业目录；pipeline 的
        --input 传根目录（判别器矩阵 66 根级 rel 需要），若在此处 rglob
        根目录会把职业外的文件混入去重，污染 canonical 集。canonical 集
        存 resolve() 后的绝对路径，process 用 resolve() 比较，兼容相对/绝对
        两种 --input 传法。
        """
        ...
        # 去重：相同内容只保留一份规范副本（字节级哈希，与老脚本同口径）
        hash_to_path: Dict[str, Path] = {}
        for file_path in sorted(INPUT_DIR.rglob("*.md")):
            try:
                ...
                # 读文件失败的文件被排除出 canonical 集，process 阶段自然跳过
                continue
        ...
        self._canonical_paths = {p.resolve() for p in canonical_files}

    def process(self, file_path: Path) -> List[StandardChunk]:
        """逐文件分派老逻辑；非 canonical 副本跳过（与老脚本去重等价）。

        统一用 resolve() 后的绝对路径：pipeline --input 既可能是绝对路径
        也可能是相对路径（相对 cwd），relative_to(INPUT_DIR) 只有在同基底
        时才成立，resolve 一次后两者等价。
        """
        if self._ctx is None or self._canonical_paths is None:
            raise RuntimeError("load_resources 未调用")
        abs_path = file_path.resolve()
        if abs_path not in self._canonical_paths:
            return []
        rel_path = abs_path.relative_to(INPUT_DIR)
        processor = create_processor(abs_path, rel_path, self._ctx)
        return [self._to_standard(c) for c in processor.run()]

    def _to_standard(self, c: Chunk) -> StandardChunk:
        """老 Chunk → 框架 StandardChunk（《职业法术模块重整方案》§3 映射）。

        顶层 1:1：chunk_id/doc_id/category/component_type/title/text/
        chm_toc_path/aliases/book_abbreviation；book_source→book_name_cn、
        book_source_english→book_name_en；其余业务字段平铺进 metadata；
        新增顶层 source_confidence（Provider 链最佳置信度）。
        """
        return StandardChunk(
            ...
            title=c.title,
            text=c.text,
            book_abbreviation=c.book_abbreviation,
            book_name_cn=c.book_source,
            book_name_en=c.book_source_english,
            source_confidence=c.source_confidence,
            chm_toc_path=c.chm_toc_path,
            aliases=list(c.aliases),
            metadata={
                "class_name": c.class_name,
                "archetype_name": c.archetype_name,
                "feature_subtype": c.feature_subtype,
                "source_paths": list(c.source_paths),
                "heading_breadcrumbs": list(c.heading_breadcrumbs),
                "source_markers": list(c.source_markers),
                "terms": list(c.terms),
...
# 模块加载时自动注册（discover_processors 触发）
register("profession", ProfessionProcessor, _profession_file_condition)
