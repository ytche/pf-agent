#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""整理 再探经典宝藏（Classic Treasures Revisited）内容到已有分类目录。"""
...
SRC_DIR = BASE / "pf_rules_md/未整理/再探经典宝藏CTR"
ORG_DIR = BASE / "pf_rules_md_organized"
REPORT_PATH = BASE / "ClassicTreasuresRevisited_reorganization_report.md"

SOURCE_BOOK = "再探经典宝藏（Classic Treasures Revisited）"
SOURCE_BOOK_SHORT = "再探经典宝藏CTR"
...
def main() -> None:
    ...
    # 1. 特殊魔法武器
    block = clean_aggregate((SRC_DIR / "特殊魔法武器1.md").read_text(encoding="utf-8"))
    ...
    if block:
        process_aggregate(
            ...
            ORG_DIR / "装备_魔法物品" / f"{SOURCE_BOOK_SHORT}_特殊魔法武器.md",
            f"{SOURCE_BOOK_SHORT} 特殊魔法武器",
            f"## {SOURCE_BOOK_SHORT} 特殊魔法武器",
            block=block,
        )

    # 2. 物品
    block = clean_aggregate((SRC_DIR / "物品7.md").read_text(encoding="utf-8"))
    block = normalize_block(block)
    if block:
        process_aggregate(
            report,
            ...
            "装备/物品",
            "wondrous_item",
            ORG_DIR / "装备_魔法物品" / f"{SOURCE_BOOK_SHORT}_物品.md",
            f"{SOURCE_BOOK_SHORT} 物品",
            f"## {SOURCE_BOOK_SHORT} 物品",
            block=block,
        )

    # 3. 专长
    block = clean_aggregate((SRC_DIR / "专长20.md").read_text(encoding="utf-8"))
    block = normalize_block(block)
    if block:
        process_aggregate(
            report,
