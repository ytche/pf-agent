#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""整理 第一世界，妖精之国（The First World, Realm of the Fey）内容到已有分类目录。

源目录：pf_data/phase1/pf_rules_md/未整理/第一世界，妖精之国TFWRotF
...
SRC_DIR = BASE / "pf_rules_md/未整理/第一世界，妖精之国TFWRotF"
ORG_DIR = BASE / "pf_rules_md_organized"
REPORT_PATH = BASE / "FirstWorldRealmOfTheFey_reorganization_report.md"

SOURCE_BOOK = "第一世界，妖精之国（The First World, Realm of the Fey）"
SOURCE_BOOK_SHORT = "第一世界TFWRotF"
...
def main() -> None:
    ...
    # 1. 专长（古老者的服从仪典 + 妖精仪典专长）
    feat_block = clean_aggregate((SRC_DIR / "古老者的服从仪典.md").read_text(encoding="utf-8"))
    ...
    # 2. 施法规则
    spell_rule_block = clean_aggregate((SRC_DIR / "施法规则.md").read_text(encoding="utf-8"))
    spell_rule_block = _collapse_blank_lines(spell_rule_block.strip())
    if spell_rule_block:
        process_aggregate(
            report,
            "施法规则.md",
            "规则",
            "first_world_spellcasting",
            ORG_DIR / "规则" / "第一世界TFWRotF_第一世界施法规则.md",
            "第一世界TFWRotF 第一世界施法规则",
            "## 第一世界TFWRotF 第一世界施法规则",
            block=spell_rule_block,
        )

    # 3. 妖精誓缚者进阶职业
    class_block = clean_aggregate((SRC_DIR / "妖精誓缚者.md").read_text(encoding="utf-8"))
    class_block = _normalize_stars(class_block)
    class_block = _collapse_blank_lines(class_block.strip())
    if class_block:
        process_aggregate(
            report,
            "妖精誓缚者.md",
            "职业选项 → 进阶职业",
            "feysworn",
            ORG_DIR / "职业" / "进阶职业" / "第一世界TFWRotF_妖精誓缚者.md",
            "第一世界TFWRotF 妖精誓缚者",
            "## 第一世界TFWRotF 妖精誓缚者",
            block=class_block,
        )
