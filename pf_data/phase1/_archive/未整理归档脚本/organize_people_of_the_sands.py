#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""整理 沙之子民（People of the Sands）内容到已有分类目录。

源目录：pf_data/phase1/pf_rules_md/未整理/沙之子民
...
SRC_DIR = BASE / "pf_rules_md/未整理/沙之子民"
ORG_DIR = BASE / "pf_rules_md_organized"
REPORT_PATH = BASE / "PeopleOfTheSands_reorganization_report.md"

SOURCE_BOOK = "沙之子民（People of the Sands）"
SOURCE_BOOK_SHORT = "沙之子民PotS"
...
def main() -> None:
    ...
    # 1. 奥斯里昂的神祇
    deity_block = clean_aggregate((SRC_DIR / "奥斯里昂的神祇.md").read_text(encoding="utf-8"))
    deity_block = _collapse_blank_lines(deity_block.strip())
    if deity_block:
        process_aggregate(
            report,
            "奥斯里昂的神祇.md",
            "信仰 → 神祇",
            "osirion_gods",
            ORG_DIR / "信仰" / "沙之子民PotS_奥斯里昂的神祇.md",
            "沙之子民PotS 奥斯里昂的神祇",
            "## 沙之子民PotS 奥斯里昂的神祇",
            block=deity_block,
        )
    ...
    # 2. 背景特性
    trait_block = clean_aggregate((SRC_DIR / "背景特性23.md").read_text(encoding="utf-8"))
    ...
    # 3. 盗贼天赋
    talent_block = clean_aggregate((SRC_DIR / "盗贼天赋1.md").read_text(encoding="utf-8"))
    talent_block = _collapse_blank_lines(talent_block.strip())
    if talent_block:
        process_aggregate(
            report,
            "盗贼天赋1.md",
            "职业选项 → 盗贼",
            "rogue_talents",
            ORG_DIR / "职业" / "基础职业" / "盗贼" / "沙之子民PotS_盗贼天赋.md",
            "沙之子民PotS 盗贼天赋",
            "## 沙之子民PotS 盗贼天赋",
            block=talent_block,
        )

    # 4. 炼金术师科研发现 + 苏比亚炼金师变体 合并到炼金术师
    alchemist_blocks: list[str] = []
    for src_file in ["炼金术师科研发现1.md", "苏比亚炼金师.md"]:
        ...
        if block:
            alchemist_blocks.append(block)
    if alchemist_blocks:
        alchemist_block = _collapse_blank_lines("\n\n".join(alchemist_blocks))
        process_aggregate(
            report,
            "炼金术师科研发现1.md+苏比亚炼金师.md",
            "职业选项 → 炼金术师",
            "alchemist_options",
            ORG_DIR / "职业" / "基础职业" / "炼金术师" / "沙之子民PotS_炼金术师.md",
            "沙之子民PotS 炼金术师",
            "## 沙之子民PotS 炼金术师",
            block=alchemist_block,
        )

    # 5. 牧师子域和奥斯里昂神祇
    domain_block = clean_aggregate((SRC_DIR / "牧师子域和奥斯里昂神祇.md").read_text(encoding="utf-8"))
    domain_block = _collapse_blank_lines(domain_block.strip())
    if domain_block:
        process_aggregate(
            report,
            "牧师子域和奥斯里昂神祇.md",
            "职业选项 → 牧师",
            "cleric_subdomains",
            ORG_DIR / "职业" / "核心职业" / "牧师" / "沙之子民PotS_牧师子域.md",
            "沙之子民PotS 牧师子域",
            "## 沙之子民PotS 牧师子域",
            block=domain_block,
        )
    ...
    # 6. 术士血统
    bloodline_block = clean_aggregate((SRC_DIR / "术士血统5.md").read_text(encoding="utf-8"))
    ...
    if bloodline_block:
        process_aggregate(
            ...
            "职业选项 → 术士",
            "sorcerer_bloodline",
            ORG_DIR / "职业" / "核心职业" / "术士" / "沙之子民PotS_术士血统.md",
            "沙之子民PotS 术士血统",
            "## 沙之子民PotS 术士血统",
    ...
    # 7. 物品
    item_block = clean_aggregate((SRC_DIR / "物品19.md").read_text(encoding="utf-8"))
    ...
    if item_block:
        process_aggregate(
            ...
            "装备/物品 → 奇物",
            "magic_items",
            ORG_DIR / "装备_魔法物品" / "魔法物品" / "奇物" / "沙之子民PotS_奇物.md",
            "沙之子民PotS 奇物",
            "## 沙之子民PotS 奇物",
            block=item_block,
        )
    ...
    # 8. 专长
    feat_block = clean_aggregate((SRC_DIR / "专长44.md").read_text(encoding="utf-8"))
