#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""整理 Pathfinder Roleplaying Game: Niobe 内容到已有分类目录。"""
...
REPORT_PATH = BASE / "Niobe_reorganization_report.md"

SOURCE_BOOK = "Pathfinder Roleplaying Game: Niobe"
SOURCE_BOOK_SHORT = "Niobe"
...
def main() -> None:
    ...
    text = clean_aggregate((SRC_DIR / "Pathfinder_Roleplaying_Game__Niobe.md").read_text(encoding="utf-8"))
    text = normalize_block(text)

    split = split_at_heading(text, r"\n\s*\*\*受祝猎手")
    if split is None:
        report["warnings"].append("未找到受祝猎手分隔点，整段写入游侠变体")
        archetype_block = text
        feat_block = ""
    else:
        archetype_block, feat_block = split

    if archetype_block:
        process_aggregate(
            report,
            "Pathfinder_Roleplaying_Game__Niobe.md",
            "职业选项 → 游侠变体",
            "blood_hunter",
            ORG_DIR / "职业" / "核心职业" / "游侠" / f"{SOURCE_BOOK_SHORT}_游侠变体.md",
            f"{SOURCE_BOOK_SHORT} 游侠变体",
            f"## {SOURCE_BOOK_SHORT} 寻血猎手",
