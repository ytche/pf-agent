#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""整理 正义编年史（Chronicle of the Righteous）内容到已有分类目录。"""
...
SRC_DIR = BASE / "pf_rules_md/未整理/正义编年史"
ORG_DIR = BASE / "pf_rules_md_organized"
REPORT_PATH = BASE / "ChronicleOfTheRighteous_reorganization_report.md"

SOURCE_BOOK = "正义编年史（Chronicle of the Righteous）"
SOURCE_BOOK_SHORT = "正义编年史"
...
def main() -> None:
    ...
    # 1. 禁欲仪式规则
    block = clean_aggregate((SRC_DIR / "page_879.md").read_text(encoding="utf-8"))
    ...
    if block:
        process_aggregate(
            ...
            "page_879.md",
            "规则",
            "rituals_of_mortification",
            ORG_DIR / "规则" / f"{SOURCE_BOOK_SHORT}_禁欲仪式.md",
            f"{SOURCE_BOOK_SHORT} 禁欲仪式",
            f"## {SOURCE_BOOK_SHORT} 禁欲仪式",
            block=block,
        )

    # 2. 至高天神使（Empyreal Lords）页码文件
    empyreal_dir = SRC_DIR / "至高天神使(Lords of the Empyrean)"
    target_path = ORG_DIR / "信仰" / f"{SOURCE_BOOK_SHORT}_至高天神使.md"
    for page_file in sorted(empyreal_dir.glob("page_*.md")):
        block = clean_aggregate(page_file.read_text(encoding="utf-8"))
        block = normalize_block(block)
        if not block:
            continue
        process_aggregate(
            report,
            f"至高天神使(Lords of the Empyrean)/{page_file.name}",
            "信仰",
            page_file.stem,
            target_path,
            f"{SOURCE_BOOK_SHORT} 至高天神使",
            f"## {SOURCE_BOOK_SHORT} 至高天神使（{page_file.stem}）",
            block=block,
        )
