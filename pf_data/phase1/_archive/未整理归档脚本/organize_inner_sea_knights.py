#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""整理 内海骑士（Knights of the Inner Sea）KIS 内容到已有分类目录。

源目录：pf_data/phase1/pf_rules_md/未整理/内海骑士
...
SRC_DIR = BASE / "pf_rules_md/未整理/内海骑士"
ORG_DIR = BASE / "pf_rules_md_organized"
REPORT_PATH = BASE / "InnerSeaKnights_reorganization_report.md"

SOURCE_BOOK = "内海骑士（Knights of the Inner Sea）KIS"
SOURCE_BOOK_SHORT = "KIS"
...
def split_at_heading(text: str, pattern: re.Pattern) -> tuple[str, str]:
    """按第一个匹配 pattern 的标题行将文本切分为 (之前, 标题及之后)。"""
    lines = text.splitlines()
...
def main() -> None:
    ...
    process_aggregate(
        ...
        "背景特性",
        "__aggregate__",
        ORG_DIR / "背景特性" / "内海骑士KIS_背景特性.md",
        "内海骑士 KIS 背景特性",
        "## 内海骑士 KIS 背景特性",
    )

    # 3. 侍从：拆分为专长与变体
    squire_text = clean_aggregate((SRC_DIR / "侍从.md").read_text(encoding="utf-8"))
    feat_part, archetype_part = split_at_heading(squire_text, re.compile(r"^\*\*侍从变体"))
    ...
    # 4. 魔法物品：拆分为武器与奇物
    magic_text = clean_aggregate((SRC_DIR / "魔法物品5.md").read_text(encoding="utf-8"))
    weapons_part, wondrous_part = split_at_heading(magic_text, re.compile(r"^\*\*奇物\*\*$"))
    ...
    process_aggregate(
        ...
        "装备",
        "特殊魔法武器",
        ORG_DIR / "装备_魔法物品" / "内海骑士KIS_魔法武器.md",
        "内海骑士 KIS 特殊魔法武器",
        "## 内海骑士 KIS 特殊魔法武器",
        text=weapons_part,
    )
    ...
    process_aggregate(
        ...
        "装备",
        "奇物",
        ORG_DIR / "装备_魔法物品" / "内海骑士KIS_奇物.md",
        "内海骑士 KIS 奇物",
        "## 内海骑士 KIS 奇物",
        text=wondrous_part,
    )

    # 5. 物品5.md 与 魔法物品5.md 中的扈从盛装重复，跳过
    report["skipped"].append("装备：物品5.md（地狱扈从的战甲 / Armiger's Panoply）与 魔法物品5.md 中的扈从盛装为同一物品重复翻译，跳过")
