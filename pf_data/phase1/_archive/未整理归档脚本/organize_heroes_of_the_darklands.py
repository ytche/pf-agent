#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""整理 幽暗地域英雄（Heroes of the Darklands）内容到已有分类目录。"""

from __future__ import annotations

import re
from datetime import datetime
from pathlib import Path

BASE = Path("/Users/chezi/code/java/pf_agent/pf_data/phase1")
SRC_DIR = BASE / "pf_rules_md/未整理/幽暗地域英雄HotD"
ORG_DIR = BASE / "pf_rules_md_organized"
REPORT_PATH = BASE / "HeroesOfTheDarklands_reorganization_report.md"

SOURCE_BOOK = "幽暗地域英雄（Heroes of the Darklands）"
SOURCE_BOOK_SHORT = "幽暗地域英雄HotD"


def write_text_preserve(path: Path, text: str, line_ending: str = "\n") -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(text.replace("\n", line_ending), encoding="utf-8")


def detect_line_ending(path: Path) -> str:
    with open(path, "rb") as f:
        f.seek(0, 2)
        size = f.tell()
        f.seek(max(0, size - 4096))
        tail = f.read()
    if b"\r\n" in tail:
        return "\r\n"
    return "\n"


def safe_append_to_file(
    path: Path,
    marker: str,
    heading: str,
    source_annotation: str,
    block: str,
) -> None:
    if not path.exists():
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_bytes(b"")
    with open(path, "rb") as f:
        content = f.read()
    if marker.encode("utf-8") in content:
        raise FileExistsError(f"{path} already contains {marker}")
    le = detect_line_ending(path)
    if not content.endswith(b"\n"):
        suffix = le * 2
    elif content.endswith(le.encode("utf-8")):
        if len(content) >= len(le.encode("utf-8")) * 2 and content.endswith(
            (le * 2).encode("utf-8")
        ):
            suffix = ""
        else:
            suffix = le
    else:
        suffix = le * 2
    entry = f"{marker}\n{heading}\n{source_annotation}\n{block}\n\n"
    entry = entry.replace("\n", le)
    with open(path, "ab") as f:
        f.write(suffix.encode("utf-8"))
        f.write(entry.encode("utf-8"))


def make_source_annotation(source_file: str, l3: str = "") -> str:
    l3_part = f" → {l3}" if l3 else ""
    return f"> 来源：{SOURCE_BOOK}，页码见原书，未整理 → {SOURCE_BOOK_SHORT}{l3_part}"


def make_hidden_marker(source_file: str, entry_name: str) -> str:
    return f"<!-- {SOURCE_BOOK_SHORT}-source:{source_file}:{entry_name} -->"


def remove_image_links(text: str) -> str:
    return re.sub(r"!\[.*?\]\(.*?\)", "", text)


def clean_translator_url(text: str) -> str:
    lines = text.splitlines()
    while lines:
        first = lines[0].strip()
        if (
            first.startswith("[http")
            or first.startswith("**[http")
            or re.match(r"^https?://", first)
            or re.match(r"^\*\*\[https?://.*\]\(.*\)\*\*$", first)
            or "整理者" in first
            or "译者" in first
            or first == ""
        ):
            lines = lines[1:]
        else:
            break
    return "\n".join(lines)


def clean_leading_image(text: str) -> str:
    lines = text.splitlines()
    while lines and re.match(r"^\s*!\[.*\]\(.*\)\s*$", lines[0].strip()):
        lines = lines[1:]
    while lines and lines[0].strip() == "":
        lines = lines[1:]
    return "\n".join(lines)


def clean_aggregate(text: str) -> str:
    text = text.replace("\r\n", "\n")
    text = remove_image_links(text)
    text = clean_translator_url(text)
    text = clean_leading_image(text)
    return text


def strip_trailing_artifacts(block: str) -> str:
    lines = block.splitlines()
    while lines and (
        lines[-1].strip() == ""
        or re.match(r"^\*+$", lines[-1].strip())
        or re.match(r"^\|", lines[-1].strip())
        or re.match(r"^\|?\s*---", lines[-1].strip())
        or lines[-1].strip() == "---"
    ):
        lines.pop()
    return "\n".join(lines)


def _collapse_blank_lines(text: str) -> str:
    return re.sub(r"\n{3,}", "\n\n", text)


def _normalize_stars(text: str) -> str:
    text = re.sub(
        r"\*\*([^*]+?)\*\*\s*[（(]\s*\*([^*]+?)\*\s*[）)]",
        r"**\1（\2）**",
        text,
    )
    text = re.sub(
        r"\*\*([^*（）()]+?)\s*[（(]([^）()]+)[）)]\*\*",
        r"**\1（\2）**",
        text,
    )
    return text


def process_aggregate(
    report: dict,
    src_name: str,
    l3: str,
    entry_name: str,
    target_path: Path,
    target_title: str,
    heading: str,
    block: str | None = None,
) -> None:
    if block is None:
        block = clean_aggregate((SRC_DIR / src_name).read_text(encoding="utf-8"))

    target_path.parent.mkdir(parents=True, exist_ok=True)
    if not target_path.exists():
        write_text_preserve(target_path, f"# {target_title}\n\n", "\n")

    marker = make_hidden_marker(src_name, entry_name)
    if marker in target_path.read_text(encoding="utf-8"):
        report["skipped"].append(f"{l3}已存在: {src_name}:{entry_name}")
        return

    block = strip_trailing_artifacts(block)
    safe_append_to_file(target_path, marker, heading, make_source_annotation(src_name, l3), block)
    report["merged"].append({"type": l3, "name": target_title, "target": str(target_path.relative_to(ORG_DIR))})


def write_report(report: dict) -> None:
    lines = [f"# {SOURCE_BOOK} 整理报告\n", f"\n生成时间：{datetime.now().isoformat()}\n"]
    lines.append("\n## 整理内容\n")
    for item in report["merged"]:
        lines.append(f"- **{item['type']}**：{item['name']} → `{item['target']}`")
    lines.append("\n## 跳过项\n")
    if report["skipped"]:
        for item in report["skipped"]:
            lines.append(f"- {item}")
    else:
        lines.append("\n无\n")
    lines.append("\n## 警告\n")
    if report["warnings"]:
        for item in report["warnings"]:
            lines.append(f"- {item}")
    else:
        lines.append("\n无\n")
    write_text_preserve(REPORT_PATH, "\n".join(lines))


CLASS_PATHS = {
    "操念使": ("职业/异能冒险（Occult Adventures）/操念使", "操念使"),
    "德鲁伊": ("职业/核心职业/德鲁伊", "德鲁伊"),
    "炼金术师": ("职业/基础职业/炼金术师", "炼金术师"),
    "女巫": ("职业/基础职业/女巫", "女巫"),
    "骑士": ("职业/基础职业/骑士", "骑士"),
    "骑将": ("职业/基础职业/骑士", "骑士"),
    "萨满": ("职业/混合职业/萨满", "萨满"),
    "杀手": ("职业/混合职业/杀手", "杀手"),
    "野蛮人": ("职业/核心职业/野蛮人", "野蛮人"),
    "异能者": ("职业/异能冒险（Occult Adventures）/异能者", "异能者"),
    "吟游诗人": ("职业/核心职业/吟游诗人", "吟游诗人"),
}


def main() -> None:
    report: dict = {"merged": [], "skipped": [], "warnings": []}

    variant_dir = SRC_DIR / "变体_选项"
    for filename in sorted(variant_dir.iterdir()):
        if not filename.is_file() or filename.suffix != ".md":
            continue

        # 文件名形如 "操念使5.md"，提取职业名
        class_name = re.sub(r"\d+$", "", filename.stem)
        if class_name not in CLASS_PATHS:
            report["warnings"].append(f"未识别的职业文件：{filename.name}")
            continue

        rel_dir, class_key = CLASS_PATHS[class_name]
        block = clean_aggregate(filename.read_text(encoding="utf-8"))
        block = _normalize_stars(block)
        block = _collapse_blank_lines(block.strip())
        if not block:
            report["warnings"].append(f"文件为空：{filename.name}")
            continue

        target_path = ORG_DIR / rel_dir / f"{SOURCE_BOOK_SHORT}_{class_key}变体.md"
        process_aggregate(
            report,
            f"变体_选项/{filename.name}",
            f"职业选项 → {class_name}",
            f"{class_key}_archetype",
            target_path,
            f"{SOURCE_BOOK_SHORT} {class_name}变体",
            f"## {SOURCE_BOOK_SHORT} {class_name}变体",
            block=block,
        )

    # 特殊材料
    block = clean_aggregate((SRC_DIR / "特殊材料.md").read_text(encoding="utf-8"))
    block = _normalize_stars(block)
    block = _collapse_blank_lines(block.strip())
    if block:
        process_aggregate(
            report,
            "特殊材料.md",
            "装备/特殊材料",
            "special_material",
            ORG_DIR / "装备_魔法物品" / "幽暗地域英雄HotD_特殊材料.md",
            "幽暗地域英雄HotD 炽辐晶",
            "## 幽暗地域英雄HotD 炽辐晶",
            block=block,
        )

    # 物品
    block = clean_aggregate((SRC_DIR / "物品18.md").read_text(encoding="utf-8"))
    block = _normalize_stars(block)
    block = _collapse_blank_lines(block.strip())
    if block:
        process_aggregate(
            report,
            "物品18.md",
            "装备/物品",
            "equipment",
            ORG_DIR / "装备_魔法物品" / "幽暗地域英雄HotD_物品.md",
            "幽暗地域英雄HotD 物品",
            "## 幽暗地域英雄HotD 物品",
            block=block,
        )

    write_report(report)
    print(f"整理完成。合并 {len(report['merged'])} 项，跳过 {len(report['skipped'])} 项，警告 {len(report['warnings'])} 项。")


if __name__ == "__main__":
    main()
