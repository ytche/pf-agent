...
SRC_DIR = BASE / "pf_rules_md/未整理/Planes of Power"
ORG_DIR = BASE / "pf_rules_md_organized"
REPORT_PATH = BASE / "PlanesOfPower_reorganization_report.md"

SOURCE_BOOK = "Planes of Power"
SOURCE_BOOK_SHORT = "PlanesOfPowerPoP"
...
def main() -> None:
    ...
    # 1. 变体/操念使2.md → 操念使变体
    block = clean_aggregate((SRC_DIR / "变体" / "操念使2.md").read_text(encoding="utf-8"))
    ...
    if block:
        process_aggregate(
            ...
            "职业选项 → 操念使变体",
            "element_purist",
            ORG_DIR / "职业" / "异能冒险（Occult Adventures）" / "操念使" / f"{SOURCE_BOOK_SHORT}_操念使变体.md",
            f"{SOURCE_BOOK_SHORT} 操念使变体",
            f"## {SOURCE_BOOK_SHORT} 元念使",
            block=block,
        )

    # 2. 物品22.md → 魔法物品
    block = clean_aggregate((SRC_DIR / "物品22.md").read_text(encoding="utf-8"))
    block = normalize_block(block)
    if block:
        process_aggregate(
            report,
            "物品22.md",
            "装备/魔法物品",
            "rift_rending_bracers",
            ORG_DIR / "装备_魔法物品" / f"{SOURCE_BOOK_SHORT}_装备.md",
            f"{SOURCE_BOOK_SHORT} 装备",
            f"## {SOURCE_BOOK_SHORT} 划空裂隙护腕",
            block=block,
        )
