#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""整理 巨人再临（Giants Revisited）内容到已有分类目录。"""
...
SRC_DIR = BASE / "pf_rules_md/未整理/巨人再临Giants Revisited"
ORG_DIR = BASE / "pf_rules_md_organized"
REPORT_PATH = BASE / "GiantsRevisited_reorganization_report.md"

SOURCE_BOOK = "巨人再临（Giants Revisited）"
SOURCE_BOOK_SHORT = "巨人再临GR"
...
def main() -> None:
    ...
    # 1. 魔法物品
    block = clean_aggregate((SRC_DIR / "魔法物品14.md").read_text(encoding="utf-8"))
    ...
    if block:
        process_aggregate(
            ...
            "装备/魔法物品",
            "all_seeing_eye",
            ORG_DIR / "装备_魔法物品" / f"{SOURCE_BOOK_SHORT}_魔法物品.md",
            f"{SOURCE_BOOK_SHORT} 全视之眼",
            f"## {SOURCE_BOOK_SHORT} 全视之眼",
            block=block,
        )

    # 2. 专长
    block = clean_aggregate((SRC_DIR / "专长54.md").read_text(encoding="utf-8"))
    block = normalize_block(block)
    if block:
        process_aggregate(
            report,
