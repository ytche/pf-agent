...
REPORT_PATH = BASE / "UnknownSource_reorganization_report.md"

SOURCE_BOOK = "出处未明"
SOURCE_BOOK_SHORT = "出处未明"
...
def main() -> None:
    ...
    block = clean_aggregate((SRC_DIR / "出处未明.md").read_text(encoding="utf-8"))
    ...
    if block:
        process_aggregate(
            ...
            "职业选项 → 唤魂师变体",
            "spirit_fuse",
            ORG_DIR / "职业" / "异能冒险（Occult Adventures）" / "唤魂师" / f"{SOURCE_BOOK_SHORT}_唤魂师变体.md",
            f"{SOURCE_BOOK_SHORT} 唤魂师变体",
            f"## {SOURCE_BOOK_SHORT} 魂线熔接者",
            block=block,
        )
