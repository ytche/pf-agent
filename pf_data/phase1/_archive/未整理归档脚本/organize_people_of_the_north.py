#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""整理 北地居民（People of the North）内容到已有分类目录。

源目录：pf_data/phase1/pf_rules_md/未整理/北地居民
...
SRC_DIR = BASE / "pf_rules_md/未整理/北地居民"
ORG_DIR = BASE / "pf_rules_md_organized"
REPORT_PATH = BASE / "PeopleOfTheNorth_reorganization_report.md"

SOURCE_BOOK = "北地居民（People of the North）"
SOURCE_BOOK_SHORT = "北地居民PotN"
...
def main() -> None:
    ...
    # 1. 先知凛冬秘示域
    mystery_block = clean_aggregate((SRC_DIR / "page_533.md").read_text(encoding="utf-8"))
    mystery_block = _normalize_stars(mystery_block)
    ...
    if mystery_block:
        process_aggregate(
            ...
            "page_533.md",
            "职业选项 → 先知",
            "oracle_mystery_winter",
            ORG_DIR / "职业" / "基础职业" / "先知" / "北地居民PotN_先知秘示域.md",
            "北地居民PotN 先知秘示域",
            "## 北地居民PotN 先知秘示域",
            block=mystery_block,
        )
    ...
    # 2. 背景特性
    trait_block = clean_aggregate((SRC_DIR / "背景特性13.md").read_text(encoding="utf-8"))
    ...
    # 3. 专长
    feat_block = clean_aggregate((SRC_DIR / "专长18.md").read_text(encoding="utf-8"))
