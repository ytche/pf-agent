#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""整理 内海酒馆（Inner Sea Taverns）ISTav 内容到已有分类目录。

源目录：pf_data/phase1/pf_rules_md/未整理/内海酒馆ISTav
...
SRC_DIR = BASE / "pf_rules_md/未整理/内海酒馆ISTav"
ORG_DIR = BASE / "pf_rules_md_organized"
REPORT_PATH = BASE / "InnerSeaTaverns_reorganization_report.md"

SOURCE_BOOK = "内海酒馆（Inner Sea Taverns）ISTav"
SOURCE_BOOK_SHORT = "ISTav"
...
def split_page_1320(text: str) -> tuple[str, str]:
    """将 page_1320.md 切分为调酒师与保镖两部分。"""
    lines = text.splitlines()
    split_idx = None
    for i, line in enumerate(lines):
        if re.search(r"保镖（Bouncer）", line):
            split_idx = i
            break
...
def main() -> None:
    ...
    # 1. 职业变体
    page_1320_text = clean_aggregate((SRC_DIR / "page_1320.md").read_text(encoding="utf-8"))
    mixologist_block, bouncer_block = split_page_1320(page_1320_text)
    ...
    process_aggregate(
        ...
        "职业变体",
        "保镖",
        ORG_DIR / "职业" / "混合职业" / "拳师" / "page_102.md",
        "保镖（Bouncer）【拳师变体】",
        "## 保镖（Bouncer）【拳师变体】",
        block=bouncer_block,
    )
