#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""整理 再探死灵（Undead Revisited）内容到已有分类目录。"""
...
SRC_DIR = BASE / "pf_rules_md/未整理/再探死灵Undead Revisited"
ORG_DIR = BASE / "pf_rules_md_organized"
REPORT_PATH = BASE / "UndeadRevisited_reorganization_report.md"

SOURCE_BOOK = "再探死灵（Undead Revisited）"
SOURCE_BOOK_SHORT = "再探死灵UndeadRevisited"
...
def main() -> None:
    ...
    block = clean_aggregate((SRC_DIR / "制造不死生物.md").read_text(encoding="utf-8"))
    ...
    if block:
        process_aggregate(
            ...
            "规则",
            "create_undead",
            ORG_DIR / "规则" / f"{SOURCE_BOOK_SHORT}_制造不死生物.md",
            f"{SOURCE_BOOK_SHORT} 制造不死生物",
            f"## {SOURCE_BOOK_SHORT} 制造不死生物",
            block=block,
        )
