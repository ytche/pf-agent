#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
提取 CHM 完整目录结构（修复深度计算）
根 UL 下的直接子项应该是 L1，不是 L0
"""

import argparse
import json
import re
from pathlib import Path
from collections import Counter


def parse_args():
    parser = argparse.ArgumentParser(description="提取 CHM 完整目录结构")
    parser.add_argument(
        "--hhc",
        type=Path,
        default=Path.home() / ".openclaw/workspace/pf_rules_chm/Pathfinder v2.20 SC.hhc",
        help="CHM 目录文件 (.hhc) 路径",
    )
    parser.add_argument(
        "--output-md",
        type=Path,
        default=Path(__file__).parent / "CHM_FULL_TOC.md",
        help="输出 Markdown 目录文件路径",
    )
    parser.add_argument(
        "--output-json",
        type=Path,
        default=Path(__file__).parent / "toc_mapping.json",
        help="输出 filename → toc_path 映射 JSON 路径",
    )
    parser.add_argument(
        "--md-dir",
        type=Path,
        default=Path(__file__).parent / "phase1" / "pf_rules_md",
        help="Markdown 规则文件目录，用于生成 md → toc_path 映射",
    )
    return parser.parse_args()


ARGS = parse_args()
HHC_PATH = ARGS.hhc.resolve()
OUTPUT_PATH = ARGS.output_md.resolve()
OUTPUT_JSON = ARGS.output_json.resolve()

def extract_full_toc():
    """提取完整目录结构"""
    
    with open(HHC_PATH, 'r', encoding='gbk', errors='replace') as f:
        lines = f.readlines()
    
    # 找到根 <UL>
    root_ul_line = None
    for i, line in enumerate(lines):
        if '<UL>' in line and root_ul_line is None:
            root_ul_line = i
            break
    
    print(f"根 <UL> 在第 {root_ul_line + 1} 行")
    
    # 跟踪深度（从根 UL 开始，depth=1 表示在根 UL 内）
    ul_depth = 0  # 当前 UL 嵌套深度
    toc_entries = []
    
    i = root_ul_line
    while i < len(lines):
        line = lines[i]
        
        # 检查是否同时包含 </UL> 和 <LI><OBJECT>
        has_close = '</UL>' in line
        has_object = '<OBJECT type="text/sitemap">' in line
        
        if has_close and has_object and ul_depth >= 1:
            # 先处理关闭
            ul_closes = line.count('</UL>')
            ul_depth -= ul_closes
            
            # 提取标题和页面
            title = "未知"
            page = ""
            
            for j in range(i+1, min(i+5, len(lines))):
                next_line = lines[j]
                
                name_match = re.search(r'<param name="Name" value="([^"]*)"', next_line)
                if name_match:
                    title = name_match.group(1)
                
                local_match = re.search(r'<param name="Local" value="([^"]*)"', next_line)
                if local_match:
                    page = local_match.group(1)
                
                if '</OBJECT>' in next_line:
                    break
            
            # 目录项深度 = 当前 ul_depth（在父 UL 中的层级）
            # 根 UL 内的项是 L1
            entry_depth = ul_depth
            toc_entries.append((entry_depth, title, page, i + 1))
            
            # 处理同一行中的 <UL> 开启
            ul_opens = line.count('<UL>')
            if ul_opens > 0:
                ul_depth += ul_opens
            
        else:
            # 正常处理
            ul_opens = line.count('<UL>')
            ul_closes = line.count('</UL>')
            
            if ul_opens > 0:
                ul_depth += ul_opens
            
            if '<OBJECT type="text/sitemap">' in line and ul_depth >= 1:
                title = "未知"
                page = ""
                
                for j in range(i+1, min(i+5, len(lines))):
                    next_line = lines[j]
                    
                    name_match = re.search(r'<param name="Name" value="([^"]*)"', next_line)
                    if name_match:
                        title = name_match.group(1)
                    
                    local_match = re.search(r'<param name="Local" value="([^"]*)"', next_line)
                    if local_match:
                        page = local_match.group(1)
                    
                    if '</OBJECT>' in next_line:
                        break
                
                entry_depth = ul_depth
                toc_entries.append((entry_depth, title, page, i + 1))
            
            if ul_closes > 0:
                ul_depth -= ul_closes
        
        i += 1
    
    return toc_entries

def build_toc_tree(entries):
    """构建树结构"""
    
    root = {"title": "Pathfinder 规则", "page": "", "children": [], "depth": 0, "line": 0}
    stack = [root]
    
    for depth, title, page, line_num in entries:
        node = {
            "title": title,
            "page": page,
            "children": [],
            "depth": depth,
            "line": line_num
        }
        
        # 找到父节点
        while len(stack) > 1 and stack[-1]["depth"] >= depth:
            stack.pop()
        
        stack[-1]["children"].append(node)
        stack.append(node)
    
    return root

def build_path_mapping(node, current_path=None, mapping=None, title_mapping=None):
    """递归构建 filename -> 完整目录路径 和 title -> 完整目录路径 的映射"""
    if current_path is None:
        current_path = []
    if mapping is None:
        mapping = {}
    if title_mapping is None:
        title_mapping = {}

    title = node["title"]
    page = node["page"]
    depth = node["depth"]

    if depth > 0:
        current_path = current_path + [title]
        full_path = " → ".join(current_path)
        title_mapping[title] = full_path

    if page:
        # 取文件名部分（page 可能是相对路径如 page_123.html）
        page_name = Path(page).name
        mapping[page_name] = " → ".join(current_path)

    for child in node["children"]:
        build_path_mapping(child, current_path, mapping, title_mapping)

    return mapping, title_mapping


def normalize_name(name):
    """标准化文件名或标题用于匹配"""
    name = Path(name).stem
    # 统一替换下划线和空格
    name = name.replace('_', ' ').replace(' ', '')
    # 去掉常见后缀和变体标记
    name = name.lower()
    return name


def build_md_toc_mapping(html_mapping, title_mapping, md_dir):
    """构建 Markdown 文件名 -> TOC 路径 的映射"""
    md_mapping = {}
    if not md_dir.exists():
        return md_mapping

    md_files = list(md_dir.glob("*.md"))
    for md_file in md_files:
        md_name = md_file.name
        md_stem = md_file.stem

        # 1. page_*.md 直接对应 page_*.html
        html_name = md_stem + ".html"
        if html_name in html_mapping:
            md_mapping[md_name] = html_mapping[html_name]
            continue

        # 2. 语义化文件名匹配 TOC 标题
        normalized = normalize_name(md_stem)
        for title, path in title_mapping.items():
            if normalize_name(title) == normalized:
                md_mapping[md_name] = path
                break

    return md_mapping


def print_tree(node, output_lines=None):
    """打印树为 Markdown 列表"""
    if output_lines is None:
        output_lines = []

    if node["depth"] > 0:
        indent = "  " * (node["depth"] - 1)
        page_info = f" (`{node['page']}`)" if node["page"] else ""
        line = f"{indent}- {node['title']}{page_info}"
        output_lines.append(line)

    for child in node["children"]:
        print_tree(child, output_lines)

    return output_lines

def main():
    print("=" * 60)
    print("提取 CHM 完整目录结构")
    print("=" * 60)
    
    entries = extract_full_toc()
    
    # 统计
    stats = Counter(e[0] for e in entries)
    print(f"\n各层级统计：")
    for depth in sorted(stats.keys()):
        print(f"  L{depth}: {stats[depth]} 项")
    print(f"  总计: {len(entries)} 项")
    
    # 构建树
    tree = build_toc_tree(entries)
    
    # 生成 Markdown
    output_lines = []
    output_lines.append("# Pathfinder 完整目录结构")
    output_lines.append("")
    output_lines.append("## 统计信息")
    output_lines.append("")
    output_lines.append("| 层级 | 数量 |")
    output_lines.append("|------|------|")
    for depth in sorted(stats.keys()):
        output_lines.append(f"| L{depth} | {stats[depth]} |")
    output_lines.append(f"| **总计** | **{len(entries)}** |")
    output_lines.append("")
    output_lines.append("## 完整目录")
    output_lines.append("")
    
    tree_lines = print_tree(tree)
    output_lines.extend(tree_lines)

    # 保存 Markdown
    with open(OUTPUT_PATH, 'w', encoding='utf-8') as f:
        f.write('\n'.join(output_lines))

    # 构建 filename -> 目录路径映射
    html_mapping, title_mapping = build_path_mapping(tree)
    md_mapping = build_md_toc_mapping(html_mapping, title_mapping, ARGS.md_dir)

    output_data = {
        "html_to_path": html_mapping,
        "md_to_path": md_mapping,
        "title_to_path": title_mapping,
    }
    with open(OUTPUT_JSON, 'w', encoding='utf-8') as f:
        json.dump(output_data, f, ensure_ascii=False, indent=2)

    print(f"\n✅ 完整目录已保存到: {OUTPUT_PATH}")
    print(f"   文件大小: {OUTPUT_PATH.stat().st_size:,} 字节")
    print(f"\n✅ filename -> 目录路径映射已保存到: {OUTPUT_JSON}")
    print(f"   HTML 映射数量: {len(html_mapping):,} 项")
    print(f"   Markdown 映射数量: {len(md_mapping):,} 项")
    print(f"   标题映射数量: {len(title_mapping):,} 项")

    # 显示前40行预览
    print("\n" + "=" * 60)
    print("目录预览（前40行）：")
    print("=" * 60)
    for line in tree_lines[:40]:
        print(line)
    if len(tree_lines) > 40:
        print(f"\n... 还有 {len(tree_lines) - 40} 行")

if __name__ == "__main__":
    main()
