#!/usr/bin/env python3
"""
Pathfinder CHM HTML -> Markdown 转换器（修复版）
修复：处理没有页面链接的目录项（如纯分类标题）
"""
...
def main():
    ...
    for level in sorted(level_counts.keys()):
        print(f"   L{level}: {level_counts[level]} 项")
    print()
    ...
    print(f"   目录项: {len(folder_items)} 个")
    print(f"   文件项: {len(file_items)} 个")
    print()
    ...
    # 处理未在 TOC 中的文件
    print("处理未在 TOC 中的文件...")
    toc_filenames = {i['filename'] for i in items if i['filename']}
    ...
    # 显示目录结构预览
    print(f"\n目录结构预览（前20个文件夹）:")
    folders = [d for d in DST_DIR.rglob('*') if d.is_dir()]
    for i, folder in enumerate(sorted(folders)[:20]):
        rel = folder.relative_to(DST_DIR)
        depth = len(rel.parts)
