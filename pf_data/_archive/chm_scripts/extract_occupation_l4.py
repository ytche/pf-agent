...
# 从根UL开始计算ul_depth，提取职业下的L4项（ul_depth == 4）
...
for i in range(root_ul_start, len(lines)):
    ...
    # L4项：ul_depth == 4 时的 LI OBJECT
    if ul_depth == 4 and '<LI><OBJECT' in lines[i]:
        # 获取名称和Local
        name = ''
        local = ''
        for j in range(i, min(i+5, len(lines))):
            ...
            if 'Local' in lines[j]:
                ...
                if match:
                    local = match.group(1)
        l4_items.append((i+1, name, local))

print(f'\n职业下的L4项（共{len(l4_items)}个）：')
for line_num, name, local in l4_items:
