...
print(f'\n职业下的L3项（共{len(l3_items)}个）：')
for line_num, name, local, depth in l3_items:
    local_display = local if local else '(目录)'
    print(f'  line {line_num}: {name} -> {local_display}')

print(f'\n职业下的L4项（共{len(l4_items)}个）：')
for line_num, name, local, depth in l4_items:
    local_display = local if local else '(目录)'
    print(f'  line {line_num}: {name} -> {local_display}')

# 按L2父项分组L3项
print('\n\n=== 按L2父项分组的L3项 ===')
for l2_line, l2_name, l2_local, l2_depth in l2_items:
    l2_start = l2_line - 1
    l2_end = None
    ...
    for i in range(l2_start, len(lines)):
        ul_opens = lines[i].count('<UL>')
        ul_closes = lines[i].count('</UL>')
        l2_ul_depth += ul_opens - ul_closes
        
        if l2_ul_depth == 0 and ul_closes > 0 and i > l2_start:
            l2_end = i
            break
