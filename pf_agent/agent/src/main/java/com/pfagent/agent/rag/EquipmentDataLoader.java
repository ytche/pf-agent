 ...
 * 装备向量库数据加载器
 *
 * 与 ClassDataLoader 同构：首次启动时委托 ChunkImporter 导入装备 chunks
 * （gear / weapon / armor / wondrous_item / ring / rod / staff 等组件类型），
 * 持久化到独立 JSON。统一行模型 StandardChunk + StandardChunkMapper，六个模块共用。
 */
@Component
public class EquipmentDataLoader implements CommandLineRunner {

    private static final Logger log = LoggerFactory.getLogger(EquipmentDataLoader.class);
    ...
    @Value("${pf.chunks.equipment-path}")
    private String chunksPath;

    @Value("${pf.stores.equipment-file-path}")
