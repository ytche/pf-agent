 ...
 * 技能向量库数据加载器
 *
 * 与 ClassDataLoader 同构：首次启动时委托 ChunkImporter 导入技能 chunks
 * （skill / skill_task / skill_rule / skill_index 等组件类型），持久化到独立 JSON。
...
public class SkillDataLoader implements CommandLineRunner {
    ...
    @Value("${pf.chunks.skill-path}")
    private String chunksPath;

    @Value("${pf.stores.skill-file-path}")
